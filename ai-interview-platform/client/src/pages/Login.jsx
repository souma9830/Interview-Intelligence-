  ...
  const handlePasswordReset = async (e) => {
    ...
    try {
      ...
      if (err.code === 'auth/user-not-found') {
        showToast('No account exists for that email address.', 'err');
      } else if (err.code === 'auth/too-many-requests') {
        showToast('Too many reset attempts. Try again later.', 'err');
      } else {
        showToast('Could not send password reset email.', 'err');
      }
    } finally {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '400px', padding: '0 16px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: '12px', padding: '32px' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '600', color: '#fff', margin: '0 0 4px' }}>{mode === 'signin' ? 'Sign in' : 'Reset password'}</h2>
        <p style={{ fontSize: '13px', color: '#666', margin: '0 0 24px' }}>
          {mode === 'signin' ? 'Enter your credentials to access the platform.' : 'Enter your account email and we will send a secure reset link.'}
        </p>

        <form onSubmit={mode === 'signin' ? handleSubmit : handlePasswordReset} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
          ...
          {mode === 'signin' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                ...
                <button type="button" onClick={() => { setMode('reset'); setErrors({}); }} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '12px', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>
                  Forgot password?
                </button>
              </div>
              <div style={{ position: 'relative' }}>
          ...
          {mode === 'reset' && (
            <div style={{ background: '#0d0d0d', border: '1px solid #222', borderRadius: '8px', padding: '12px', fontSize: '12px', color: '#888', lineHeight: '1.5' }}>
              Reset links are sent through the configured Firebase email action provider. Keep this tab open and return to sign in after updating your password.
            </div>
          )}
          ...
          {mode === 'signin' ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '8px' }}>
              <button type="submit" disabled={resetLoading} style={{ width: '100%', padding: '11px', background: resetLoading ? '#1a1a1a' : '#fff', color: resetLoading ? '#555' : '#000', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: resetLoading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.15s' }}>
                {resetLoading ? <><Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> Sending reset…</> : <>Send reset link <ArrowRight size={15} /></>}
              </button>
              <button type="button" onClick={() => { setMode('signin'); setErrors({}); }} style={{ width: '100%', padding: '10px', background: 'transparent', color: '#aaa', border: '1px solid #333', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>
                Back to sign in
              </button>
            </div>
          )}
